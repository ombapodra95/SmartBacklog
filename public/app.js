/* ============================================================
   Agile Kanban AI — Frontend
   ============================================================ */

// ── State ─────────────────────────────────────────────────────────────────────
let tickets        = [];
let editingId      = null;   // null = create mode, string = edit mode
let currentCriteria = [];    // live list being edited in modal
...
async function init() {
  try {
    ...
    showToast('Could not load tickets from server', 'error');
  }
  renderBoard();
}
...
function renderBoard() {
  ['todo', 'inprogress', 'done'].forEach(status => {
    ...
    if (group.length === 0) {
      col.innerHTML = `
        <div class="text-center py-8 text-slate-400 text-xs select-none">
...
function buildCard(ticket) {
  ...
  const criteriaHtml = ticket.acceptanceCriteria && ticket.acceptanceCriteria.length
    ? `<span class="text-xs text-slate-400">✓ ${ticket.acceptanceCriteria.length} criteria</span>`
    : '';

  const pointsHtml = ticket.storyPoints
  ...
  card.innerHTML = `
    <div class="absolute top-2 right-2 hidden group-hover:flex gap-1 z-10">
      <button
        ...
        class="text-xs bg-indigo-100 text-indigo-600 px-2 py-1 rounded-md hover:bg-indigo-200 font-medium"
      >Edit</button>
      <button
        onclick="handleDelete('${ticket.id}')"
        class="text-xs bg-red-100 text-red-600 px-2 py-1 rounded-md hover:bg-red-200 font-medium"
      >Del</button>
    </div>
...
async function saveTicket() {
  ...
  const payload = {
    ...
    storyPoints:        document.getElementById('ticket-points').value
                          ? parseInt(document.getElementById('ticket-points').value)
                          : null,
...
function renderCriteria() {
  ...
  currentCriteria.forEach((c, i) => {
    ...
    row.innerHTML = `
      ...
      <button onclick="removeCriteria(${i})" class="text-slate-300 hover:text-red-500 text-lg leading-none flex-shrink-0">&times;</button>
    `;
...
async function aiGenerateCriteria() {
  ...
  try {
    ...
    if (res.acceptanceCriteria) {
      currentCriteria = res.acceptanceCriteria;
      renderCriteria();
      showToast('Acceptance criteria generated');
    }
  } catch (err) {
...
async function aiEstimatePoints() {
  ...
  try {
    ...
    if (res.storyPoints) {
      ...
      showToast(`Estimated: ${res.storyPoints} story points`);
    }
  } catch (err) {
    showToast(err.message, 'error');
  } finally {
...
function showToast(msg, type = 'success') {
  ...
  inner.className = `px-4 py-3 rounded-xl shadow-lg text-sm font-medium text-white ${
